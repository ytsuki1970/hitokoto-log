...
いまは PC のフォルダにあるだけなので、スマホからは開けない。どちらかを選ぶ。

1. **GitHub Pages に置く**（まなびログと同じやり方）
   アプリのファイルだけが公開され、**記録の中身は端末内に残る**。URLを知られても記録は見えない
2. **家のWi-Fi内だけで使う**
   `start.bat` を動かして、同じWi-Fiのスマホから `http://<PCのIP>:8932/` を開く。PCを起動している間だけ
